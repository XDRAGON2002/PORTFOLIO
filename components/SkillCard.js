...
const pageVariants = {
    ...
    animate: coord => ({opacity: 1, x: 0, y: 0, scale: 1, transition : {delay: coord[0] * 1, duration: Math.random()}}),
...
const SkillCard = ({item,row,col}) => {
    ...
    return (
        <motion.div initial = "initial" animate = "animate" exit = "exit" variants = {pageVariants} custom = {[row,col]} onTapStart = {() => setShowTitle(!showTitle)} whileHover = {{scale: 1.2, transition: {duration: 0.3}}} onHoverEnd = {() => setShowTitle(false)} className = "flex-grow mx-3 my-3">
